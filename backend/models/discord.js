...
async function getOdyMembers(token) {
  // App needs to be a bot application in order to use this!
  logger.debug(`Retrieving ODY guild members with ID ${ODY_SERVER_ID}`);
  const { data } = await axios({
    method: 'GET',
    url: `https://discordapp.com/api/guilds/${ODY_SERVER_ID}/members`,
    headers: {
      Authorization: `Bearer ${token}`,
    },
    params: {
